...
uint16_t AbsoluteAddressingStrategy::GetMemoryByte() const
{
	...
	return memory.ReadShort(cpu.GetRegisterProgramCounter() + 1);
}
...
uint8_t AbsoluteAddressingStrategy::GetMemoryByteValue() const
{
	CPU& cpu = NESConsole::GetInstance()->GetCPU();
	CPUMemory& memory = NESConsole::GetInstance()->GetMemory();
	return memory.ReadByte(GetMemoryByte());
}
...
void AbsoluteYAddressingStrategy::ToString(std::string &out_string) const
{
	CPU& cpu = NESConsole::GetInstance()->GetCPU();
	CPUMemory& memory = NESConsole::GetInstance()->GetMemory();
	uint16_t memory_address = memory.ReadShort(cpu.GetRegisterProgramCounterPlus(1));
	uint16_t memory_offset = memory_address + cpu.GetRegisterY();
	uint8_t value = memory.ReadByte(memory_offset);
...
uint8_t AbsoluteYAddressingStrategy::GetMemoryByteValue() const
{
	...
	return memory.ReadByte(memory_offset);
}
...
uint8_t RelativeAddressingStrategy::GetMemoryByte() const
{
	CPU& cpu = NESConsole::GetInstance()->GetCPU();
	CPUMemory& memory = NESConsole::GetInstance()->GetMemory();
	return memory.ReadByte(cpu.GetRegisterProgramCounterPlus(1));
}
...
uint16_t JMPAbsoluteAddressingStrategy::GetMemoryByte() const
{
	CPU& cpu = NESConsole::GetInstance()->GetCPU();
	CPUMemory& memory = NESConsole::GetInstance()->GetMemory();
	return memory.ReadShort(cpu.GetRegisterProgramCounterPlus(1));
}
...
uint8_t ZeroPageAddressingStrategy::GetMemoryByteValue() const
{
	...
	uint8_t zero_page_value = memory.ReadByte((uint16_t) zero_page_address);
...
void ZeroPageXAddressingStrategy::ToString(std::string &out_string) const
{
	CPU& cpu = NESConsole::GetInstance()->GetCPU();
	CPUMemory& memory = NESConsole::GetInstance()->GetMemory();
	uint8_t memory_address = memory.ReadByte(cpu.GetRegisterProgramCounterPlus(1));
	uint8_t memory_offset = memory_address + cpu.GetRegisterX();
	uint8_t memory_value = memory.ReadByte((uint16_t) memory_offset);
...
void IndexedIndirectAddressingStrategy::ToString(std::string &out_string) const
{
	...
	uint16_t lsb = memory.ReadByte(lsb_location);
	uint16_t msb = memory.ReadByte(msb_location);
	uint16_t full_address = (msb << 8) | lsb;
	uint8_t value_at_address = memory.ReadByte(full_address);
...
uint8_t IndexedIndirectAddressingStrategy::GetMemoryByteValue() const
{
	...
	return memory.ReadByte(full_address);
}
...
uint16_t IndexedIndirectAddressingStrategy::GetValueAddress() const
{
	...
	lsb_location += cpu.GetRegisterX();
	uint8_t msb_location = lsb_location + (uint8_t) 1;
	uint16_t lsb = memory.ReadByte((uint16_t) lsb_location);
	uint16_t msb = memory.ReadByte((uint16_t) msb_location);
	uint16_t full_address = (msb << 8) | lsb;
	return full_address;
}
...
uint16_t IndirectAddressingStrategy::GetMemoryByteValue() const
{
	...
	CPUMemory& memory = NESConsole::GetInstance()->GetMemory();
	uint16_t value = GetMemoryByte();
	uint8_t lsb = memory.ReadByte(value);
	uint16_t msb = 0x0000;
	if ((value & 0x00FF) == 0xFF)
	{
		msb = memory.ReadByte(value & 0xFF00);
	}
	else
	{
		msb = memory.ReadByte(value + (uint16_t) 1);
	}
	uint16_t full_address = (msb << 8) | lsb;
	return full_address;
}
